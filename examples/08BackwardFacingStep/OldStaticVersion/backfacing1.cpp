...
// ----- Boundary conditions.
    ...
    int mid_point = Ny / 2; 
    for(int i = point[0] + mid_point, ii = mid_point + 2; 
	i <= point[1]; ++i, ++ii) {
	y_coord = y(i) - 0.5;
	...
	y_coord = y(i) - 0.5 ;
	sgrid(Nx, ii) = 0.75 * y_coord - y_coord * y_coord * y_coord;  
    }
...
// Edges 4: Dirichlet (sliding wall)
	...
	ei = bi + side_x;	
	for(int i = bi, k = 2; i < ei; ++i, ++k) {	  	  	  
//	    b(i) = -(4 * sgrid(k, 2) - 0.5 * sgrid(k, 3)) / (dy * dy) - 3 / dy;
