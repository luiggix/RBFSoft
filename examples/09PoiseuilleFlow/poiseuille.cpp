...
/* */
 ...
 // ----- Point generation 
    ...
    Vec xs = x / 1;   //  Scaled coordinates
    Vec ys = y / 1;   //  for RBF calculations
...
// ----- Inlet
    prec_t y_coord;
    for(int i = point[0], ii = 2; i <= point[1]; ++i, ++ii) {
	y_coord = y(i) - 0.5;
//	sgrid(1, ii) = 0.75 * y_coord - y_coord * y_coord * y_coord + 0.25;  
	sgrid(1, ii) = Re * ( 1.5*y_coord - 2*y_coord*y_coord*y_coord + 0.5);  
    }
...
//	sgrid(Nx, ii) = 0.75 * y_coord - y_coord * y_coord * y_coord + 0.25;
	sgrid(Nx, ii) = Re * ( 1.5*y_coord - 2*y_coord*y_coord*y_coord + 0.5);
    }
...
// ----- Write profiles of the velocity
	std::ofstream ufile0 ("u_line0.dat"),
	    ufile1 ("u_line1.dat"), 
	    ufile2 ("u_line2.dat"), 
	    ufile3 ("u_line3.dat"),
	    ufileN ("u_lineN.dat");
...
//	    ul = RBF::eval(hx*0.1,dl*i, xs, ys,lstream,RBF::MQ_1DY<prec_t,2>(c),0);
//	    ufile0 << dl * i << "\t" << ul << "\n";

	    ul = RBF::eval(hx*0.001,dl*i, xs, ys,lstream,RBF::MQ_1DY<prec_t,2>(c),0);
//	    ufile1 << ul << "\t" << dl * i << "\n";
	    ufile1 << dl * i << "\t" << ul << "\n";
	    ul = RBF::eval(hx*0.5,dl*i, xs, ys,lstream,RBF::MQ_1DY<prec_t,2>(c),0);
//	    ufile2 << ul << "\t" << dl * i << "\n";
	    ufile2 << dl * i << "\t" << ul << "\n";
	    ul = RBF::eval(hx*0.999,dl*i, xs, ys,lstream,RBF::MQ_1DY<prec_t,2>(c),0);
//	    ufile3 << ul << "\t" << dl * i << "\n";
	    ufile3 << dl * i << "\t" << ul << "\n";

//	    ul = RBF::eval(hx*0.9,dl*i, xs, ys,lstream,RBF::MQ_1DY<prec_t,2>(c),0);
//	    ufileN << dl * i << "\t" << ul << "\n";
	}
	ufile1.close();
	ufile2.close();
...
#ifdef WITH_GNUPLOT
	plotter("plot \"u_line1.dat\" w lp, \"u_line2.dat\" w lp, \"u_line3.dat\" w lp");//, \"u_line0.dat\" w  lp, \"u_lineN.dat\" w lp");
//    std::cout << "\n | Press <1> and then <enter> "; std::cin >> stop; 
#endif
...
// Values from analytical parabolic profile
/* */
    b(N-3) = -Re * 4;
    b(N-2) = Re * 4;
    b(N-1) = -Re * 4;
