 ...
 *  Lid Driven Cavity in 2D (static version).
 ...
 *  \verbatim
    % make
    % ./lid_driven1 \endverbatim   
 * The \c input1 file contains the initial data to setup the problem.
 * \par Output
 * \c xy_knots.dat coordinates of random points;
 * \c solStream.dat (x,y,u) evaluation of the stream function in a grid.
 * \c solVort.dat (x,y,u) evaluation of the vorticity function in a grid.
 ***************************************************************************
...
int main( int argc, char * argv[])
{
    ...
    std::ifstream input_file ("input1");
...
// ----- Declaration of matrices and vectors
    ...
    Mat Gstream(N, N), Gvort(N,N);  // Gram matrices for Stream and Vort
    Vec b(N);                       // Right hand-side of the systems
    Vec lstream(N), lvort(N);       // Lambda coeficients
    Vec uvel(N), vvel(N);           // Velocity
    Mat sgrid(Nx, Ny), vgrid(Nx, Ny); // Stream and Vort matrices (grid)     
...
// ----- Construction of the preconditioner
    ...
    int point[8], side_x = Nx - 2, side_y = Ny - 2;
//
// Locations of some points on the boundary of the domain
//
//           6       7
//           |       |
//           v       v   
...
// ----- Jacobi preconditioner for Vorticity
...
// ----- Source term for the streamfunction equation
	for(int j = 2; j < Ny; ++j)
...
#ifdef WITH_GNUPLOT
	plotStream("plot \"u_line.dat\" w lp");
	plotVort("plot \"v_line.dat\" w lp");
	std::cout << "\n | Press <1> and then <enter> "; std::cin >> stop; 
#endif
