...
#ifndef _RECTANGLEKNOTS_HPP_
#define _RECTANGLEKNOTS_HPP_
...
class RectangleKnots : public Knots< RectangleKnots<Tprec>, knot2D > 
{ 
...
// in each derived class.
    using Knots<RectangleKnots<prec_t>, knot2D >::N;   // Total points
    using Knots<RectangleKnots<prec_t>, knot2D >::NI;  // Interior points
    using Knots<RectangleKnots<prec_t>, knot2D >::NB;  // Boundary points
    using Knots<RectangleKnots<prec_t>, knot2D >::Dim; // Dimension
    using Knots<RectangleKnots<prec_t>, knot2D >::xyz; // Coordinates
    using Knots<RectangleKnots<prec_t>, knot2D >::max_upper_range; // used for
                                                                   // KDTREE
 ...
 *  Default constructor.
 */
    RectangleKnots() : Knots< RectangleKnots<Tprec>, knot2D > () { }
/*!
 ...
 */
    RectangleKnots(prec_t l_x, int n_x, prec_t l_y, int n_y,
		   random_t knd = U, int layer = 0)
	: Knots< RectangleKnots<Tprec>, knot2D > (),  
 ...
 * \image html  RectangleKnots.png "Knots order" width=5cm 
 * \image latex RectangleKnots.eps "Knots order" width=5cm  
 */
template<typename prec_t>
bool RectangleKnots<prec_t>::constructKnots() 
{
    prec_t dx = lx / (Nx - 1);
    prec_t dy = ly / (Ny - 1);
    prec_t x0, x1, y0, y1;
...
template<typename prec_t>
inline bool RectangleKnots<prec_t>::addSpecialPoints(iMat &P, int j, int nsp)
{
 ...
 */
template<typename prec_t>
bool RectangleKnots<prec_t>::gridKnots() 
{
    prec_t dx = lx / (Nx - 1);
    prec_t dy = ly / (Ny - 1);
    int index = 0;
